...
> 本文档让你在自己环境验证 feishu-skills-app 的 Phase 1 PoC 是否真正跑通了 v0.4 规范的核心假设：
>
...
**Windows PowerShell**：
```powershell
$env:FEISHU_APP_ID = "cli_a94c994e0438dbde"
$env:FEISHU_APP_SECRET = "7pDOYsumLaZVTvkLilXgWcPcVANnjEGq"
```

**Git Bash / Linux / Mac**：
```bash
export FEISHU_APP_ID="cli_a94c994e0438dbde"
export FEISHU_APP_SECRET="7pDOYsumLaZVTvkLilXgWcPcVANnjEGq"
```

### 2. 找到你自己的飞书 open_id

最简单的方式：在飞书电脑端打开任意聊天，浏览器开发者工具看 API 请求，或者在你已有 token 的环境跑：
```bash
node d:/stacs/claw/feishu-skills/feishu-search-user/search-user.mjs --open-id <已知任一open_id> --query "你的姓名"
```
...
```bash
cd d:/stacs/claw/feishu-skills-app
python runner.py list
```
**预期输出**（约 0.5 秒返回）：
```json
{"status": "completed", "output": {"pipelines": ["im-message"]}}
```

```bash
python runner.py im-message --open-id ou_invalid --action send
```
**预期输出**（约 1 秒返回）：
```json

#!/usr/bin/env node
/**
 * drive / execute.mjs — dispatch drive actions to tools/lib/drive.mjs.
 *
 * stdin:  { input: {action, ...args}, steps: { _constructor: { output: { access_token, ... }}}}
 * stdout: { output: <action-specific result> }
 */
...
const { ACTIONS, FeishuError } = await import(
  'file://' + path.join(TOOLS_DIR, 'lib', 'drive.mjs').replace(/\\/g, '/')
);
...
async function main() {
  ...
  const ctorOut = payload.steps?._constructor?.output || {};
  const accessToken = ctorOut.access_token;
  ...
  try {
    const result = await handler(accessToken, input);
    emit({ output: result });
  } catch (err) {

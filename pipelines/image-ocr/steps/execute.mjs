#!/usr/bin/env node
/**
 * image-ocr / execute.mjs — dispatch to tools/lib/image-ocr.mjs.
 */
...
const { ACTIONS, FeishuError } = await import(
  'file://' + path.join(TOOLS_DIR, 'lib', 'image-ocr.mjs').replace(/\\/g, '/')
);
...
async function main() {
  ...
  try {
    const result = await handler(input, null, cfg);
    emit({ output: result });
  } catch (err) {
